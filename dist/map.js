...
// A small pool of chevron markers that travel along the drawn route on a
// loop, evenly spaced and all moving at the same pace — a "flow" of arrows
// rather than a single static one, so the direction to walk reads at a
// glance without redrawing route.js on every frame. Built once, directly on
// `scene` (not `route`, which drawRoute clears and rebuilds every call) and
// just repositioned/reoriented each frame; visibility per-arrow tracks how
// many the current route actually needs.
const ARROW_POOL_SIZE=8;
const arrowPool=Array.from({length:ARROW_POOL_SIZE},()=>{
	const g=new THREE.Group();
	bar(g,[-.16,0,-.3],[0,0,0],.045,routeMat);
	bar(g,[.16,0,-.3],[0,0,0],.045,routeMat);
	g.visible=false;
	scene.add(g);
	return g;

...
// A small glowing marker that continuously walks the drawn route, looping —
// most useful now that the route defaults to just the next leg rather than
// the whole plan, since a moving marker reads "walk this way" at a glance
// instead of requiring the line itself to be traced by eye.
const walker=new THREE.Mesh(new THREE.SphereGeometry(.13,16,12),new THREE.MeshBasicMaterial({color:'#e9ffb0'}));walker.visible=false;scene.add(walker);
let routeWalk=null;
...
let activeIds=[];function drawRoute(ids){route.children.slice().forEach(o=>{route.remove(o);o.geometry?.dispose();if(o.userData.marker){o.material.map.dispose();o.material.dispose()}});ids.filter(id=>equipment.some(e=>e.id===id)).forEach(id=>{const index=ids.indexOf(id);const e=equipment.find(e=>e.id===id),c=document.createElement('canvas');c.width=c.height=96;const ctx=c.getContext('2d');ctx.fillStyle='#1a6847';ctx.beginPath();ctx.arc(48,48,43,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#c9ed9e';ctx.lineWidth=5;ctx.stroke();ctx.fillStyle='#fff';ctx.font='bold 48px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(String(index+1),48,50);const marker=new THREE.Mesh(new THREE.PlaneGeometry(.8,.8),new THREE.MeshBasicMaterial({map:new THREE.CanvasTexture(c),transparent:true,depthWrite:false}));marker.rotation.x=-Math.PI/2;marker.position.set(Math.round(e.x*2)/2,.2,e.z+2);marker.userData.marker=true;route.add(marker)});const walkPoints=[];for(let i=1;i<ids.length;i++){const from=allEquipment.find(e=>e.id===ids[i-1]),to=allEquipment.find(e=>e.id===ids[i]);if(from.floor!==currentFloor&&to.floor!==currentFloor)continue;const points=findPath(from.floor===currentFloor?from:gym.stairs,to.floor===currentFloor?to:gym.stairs);if(!walkPoints.length)walkPoints.push(points[0]);for(let j=1;j<points.length;j++){bar(route,points[j-1],points[j],.045,routeMat);walkPoints.push(points[j]);if(j%7===0){const a=new THREE.Vector3(...points[j-1]),b=new THREE.Vector3(...points[j]),d=b.clone().sub(a).normalize(),p=new THREE.Vector3(-d.z,0,d.x);bar(route,b.clone().addScaledVector(d,-.3).addScaledVector(p,.16).toArray(),b.toArray(),.045,routeMat);bar(route,b.clone().addScaledVector(d,-.3).addScaledVector(p,-.16).toArray(),b.toArray(),.045,routeMat)}}}routeWalk=null;if(walkPoints.length>1){let total=0;const cum=[0];for(let i=1;i<walkPoints.length;i++){total+=Math.hypot(walkPoints[i][0]-walkPoints[i-1][0],walkPoints[i][2]-walkPoints[i-1][2]);cum.push(total)}if(total>0)routeWalk={points:walkPoints,cum,total}}walker.visible=!!routeWalk}
...
function home(){controls.autoRotate=false;camera.position.set(42,46,54);if(gym.id==='atlas')camera.position.multiplyScalar(.84);if(container.clientWidth<600)camera.position.multiplyScalar(1.25);controls.target.set(0,0,1);$('#top-view').setAttribute('aria-pressed','false');$('#top-view').textContent=t('view2d');controls.update()}home();$('#home-view').onclick=home;
